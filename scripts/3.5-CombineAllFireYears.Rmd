...
- Remove polygon that drew as large triangle
- Remove SSFS because it is not in the Boreal or Tundra Biome
- Remove fire smaller then 200 ha following Canadian large wildfire database
- Remove any fires with mean confidence of zero based on Modis hotspots
```{r}
all_fires1 = all_fires %>% 
  filter(!(FireYr == "2014" & IDobj=="2147")) %>%
  filter((EcoCode!="SSFS")) %>% 
  filter(SizeHa >= 200) %>%
  filter(AvgConf > 0) 
```
...
22091
```{r}
n_distinct(all_fires1$FireId)
n_distinct(all_fires1$FID)
n_distinct(all_fires1$IDobj)
```

# Add Unique ID

```{r}
all_fires2 = all_fires1 %>% 
  mutate(UniqueId = random_id(n= 22091, bytes = 4)) %>%
...
```{r}
n_distinct(all_fires2$UniqueId)
```

# Write to shapefile
```{r eval=FALSE, include=FALSE}
st_write(all_fires2, "../outputs/3.5-AllFires/SiberiaFires2001-2020.shp", driver="ESRI Shapefile") 
```
